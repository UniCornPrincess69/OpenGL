...
int CEngine::Initialize(void)
{
	...
	//if (m_pSkybox == nullptr) m_pSkybox = new CSkybox(m_pDataManager);
	...
	if (m_pBlinn == nullptr) m_pBlinn = new CMaterial(m_pDataManager, "Resource Files/Shader/GlitterVertex.glsl", "Resource Files/Shader/GlitterFragment.glsl");
...
int CEngine::Run(void)
{
	while (!glfwWindowShouldClose(m_pViewport->GetWindow()))
	{
		...
		if (m_pBlinn != nullptr) m_pBlinn->AddUniformVector3("lightVec", 10.0f, 10.0f, 10.0f);
